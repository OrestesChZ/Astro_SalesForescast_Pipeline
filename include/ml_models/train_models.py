...
class ModelTrainer:
    ...
    def prepare_data(self, df: pd.DataFrame, target_col: str = 'sales',
        ...
        train_size = int(
            len(df_sorted) * (1 - self.training_config['test_size'] - self.training_config['validation_size']))
    ...
    def train_prophet(self, train_df: pd.DataFrame, val_df: pd.DataFrame,
        ...
        try:
            ...
            regressor_cols = [col for col in numeric_cols if
                              col not in [target_col, 'year', 'month', 'day', 'week', 'quarter']]
